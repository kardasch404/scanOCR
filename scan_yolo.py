...
class AdvancedIDAnalyzer:
    ...
    def __init__(self, model_root=None, auto_download_models=False, init_ocr=True):
        ...
        # ELITE PRACTICE: Thread-safety lock — prevents concurrent predict() corruption
        import threading
    ...
    def _build_ocr_engine(self):
        ...
        # ELITE PRACTICE: Warm-up inference — loads CUDA/CPU kernel caches so first real
        # request is not penalized by JIT compilation overhead.
        try:
            _dummy = np.zeros((64, 200, 3), dtype=np.uint8)
            if hasattr(engine, "predict"):
                list(engine.predict(_dummy))
            else:
                engine.ocr(_dummy)
        except Exception:
    ...
    def _preload_models(self):
        ...
        # ELITE FIX: Reuse self.ocr if already built — avoid creating a duplicate engine
        if self.ocr is None:
            self.ocr = self._build_ocr_engine()
    ...
    def extract_mrz_strictly(self, img):
        ...
        if hasattr(self.ocr, "predict"):
            # ELITE PRACTICE: Thread-safe + timeout-protected OCR call
            from concurrent.futures import ThreadPoolExecutor, TimeoutError as _FutureTimeout
            try:
                with ThreadPoolExecutor(max_workers=1) as _ex:
                    with self._ocr_lock:
                        _fut = _ex.submit(lambda: list(self.ocr.predict(mrz_bgr)))
                    result_pages = _fut.result(timeout=45)
            except _FutureTimeout:
                print("   WARNING: MRZ OCR timed out (45s) — returning empty")
                result_pages = []
            if result_pages:
                page = result_pages[0]
    ...
    def _run_ocr_on_crop(self, crop, y_offset=0, x_offset=0, max_dim=1000):
        ...
        if hasattr(self.ocr, "predict"):
            ...
            try:
                with ThreadPoolExecutor(max_workers=1) as _ex:
                    with self._ocr_lock:
                        _fut = _ex.submit(lambda: list(self.ocr.predict(crop)))
                    result_pages = _fut.result(timeout=45)
            except _FutureTimeout:
                print("   WARNING: Crop OCR timed out (45s) — returning empty")
                return []
            if not result_pages:
                return []
            page = result_pages[0]
    ...
    def targeted_ocr(self, img, doc_class_info, yolo_ctx):
        ...
        if side == 'RECTO':
            ...
            viz_boxes = self._run_ocr_on_crop(viz_crop, max_dim=1000)
        ...
        # -- CNI-V2 VERSO: VIZ above MRZ, MRZ from YOLO bbox -----
            ...
            if mrz_crop.size > 0:
                ...
                mrz_boxes = self._run_ocr_on_crop(mrz_bgr, y_offset=mrz_y1, x_offset=mrz_x1, max_dim=1000)
        ...
        # -- PASSPORT: MRZ bottom, VIZ above ----------------------
            ...
            if mrz_crop.size > 0:
                ...
                _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                mrz_bgr = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
                mrz_boxes = self._run_ocr_on_crop(mrz_bgr, y_offset=mrz_y1, max_dim=1000)
        ...
        # -- UNKNOWN: Full-page OCR (fallback) --------------------
            ...
            viz_boxes = self._run_ocr_on_crop(img, max_dim=1000)
    ...
    def extract_all_text_deep(self, img, use_preprocessing=True):
        ...
        if hasattr(self.ocr, "predict"):
            ...
            t0 = time.time()
            # ELITE PRACTICE: Thread-safe + timeout-protected full-page OCR call
            try:
                with ThreadPoolExecutor(max_workers=1) as _ex:
                    with self._ocr_lock:
                        _fut = _ex.submit(lambda: list(self.ocr.predict(img_processed)))
                    result_pages = _fut.result(timeout=90)
            except _FutureTimeout:
                print(f"   WARNING: Full-page OCR timed out (90s) — returning empty")
                return []
    ...
    def analyze(self, image_path: str) -> dict:
        ...
        # ID cards don't need more than 1400px — larger = exponentially slower (was 2000)
        MAX_EARLY_DIM = 1400
